...
resource "azurerm_route_table" "vpn_gw_rt" {
  name                          = "vpn-gw-rt"
  location                      = azurerm_resource_group.rg.location
  resource_group_name           = azurerm_resource_group.rg.name
  ...
  route {
    name                   = "Spoke1ThroughNVA"
    address_prefix         = var.spoke_1_prefix
    next_hop_type          = "VirtualAppliance"
    next_hop_in_ip_address = azurerm_network_interface.hub_nva_vm.private_ip_address
  }

  route {
    name                   = "Spoke2ThroughNVA"
    address_prefix         = var.spoke_2_prefix
    next_hop_type          = "VirtualAppliance"
    next_hop_in_ip_address = azurerm_network_interface.hub_nva_vm.private_ip_address
  }
}

resource "azurerm_subnet_route_table_association" "vpn_gw_rt" {
  subnet_id      = azurerm_subnet.hub_gw_subnet.id
  route_table_id = azurerm_route_table.vpn_gw_rt.id
}